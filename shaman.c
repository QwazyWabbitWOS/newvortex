...
qboolean curse_add(edict_t *target, edict_t *caster, int type, int curse_level, float duration)
{
	...
	*/
	...
	//decino: protect devices with firewall!
	if (type != BLESS && type != HEALING && type != DEFLECT){
	if (target->firewall_weak)
    duration *= 0.75;
    if (target->firewall_medium)
    duration *= 0.50;
    if (target->firewall_strong)
    duration *= 0.25;}
...
void CurseMessage (edict_t *caster, edict_t *target, int type, float duration, qboolean isCurse)
{
	...
	else if (target->mtype)
	{
		...
		safe_cprintf(caster, PRINT_HIGH, "%s %s with %s for %0.1f second(s)\n", typeName, V_GetMonsterName(target->mtype), curseName, duration);
	}
	else
		safe_cprintf(caster, PRINT_HIGH, "%s %s with %s for %0.1f second(s)\n", typeName, target->classname, curseName, duration);

//	if (target->firewall_weak || target->firewall_medium || target->firewall_strong)
//		gi.cprintf(caster, PRINT_HIGH, "Fire wall detected!\n", target->classname);
}
...
void Bleed (edict_t *curse)
{
	...
	//gi.dprintf("take %d health %d level %d time %.1f\n", take, curse->enemy->max_health, curse->monsterinfo.level, curse->monsterinfo.selected_time);
...
void Cmd_Healing(edict_t *ent)
{
	...
	//decino: we are a bot, we never heal other people
	if (ent->ai.is_bot)
	{
		if (!curse_add(ent, ent, HEALING, 0, duration))
		{
	...
	if (target != NULL)
	{
		...
		if (target == ent)
		{
			...
			target->heal_time = level.time + duration;

			if (target->client)
			target->client->is_poisoned = false;
		}
...
void Cmd_Bless(edict_t *ent)
{
	...
	if (target != NULL)
	{
		...
		if (target == ent)
		{
			...
			target->bless_time = level.time + duration;
		}
		else if ((target->client) && !(target->svflags & SVF_MONSTER))
		{
			safe_cprintf(target, PRINT_HIGH, "YOU HAVE BEEN BLESSED FOR %0.1f seconds!!\n", duration);
...
void Cmd_Deflect_f(edict_t *ent)
{
	...
	if (Q_strcasecmp(gi.argv(1), "self") == 0)
	{
		if (!curse_add(ent, ent, DEFLECT, 0, duration))
		{
			safe_cprintf(ent, PRINT_HIGH, "Unable to bless self.\n");
			return;
		}
	...
	{
		if (!ent->ai.is_bot)
		target = curse_Attack(ent, DEFLECT, 0, duration, false);
	}

	//decino: we are a bot, we never deflect other people
	if (ent->ai.is_bot)
	{
		if (!curse_add(ent, ent, DEFLECT, 0, duration))
		{
	...
	//	ent->myskills.abilities[DEFLECT].delay = level.time + duration + DEFLECT_DELAY;
		...
		if (target == ent)
		{
			...
			target->deflect_time = level.time + duration;
		}
		else if ((target->client) && !(target->svflags & SVF_MONSTER))
		{
			safe_cprintf(target, PRINT_HIGH, "You have been blessed with deflect for %0.1f seconds!\n\n", duration);
		...
		{
			safe_cprintf(ent, PRINT_HIGH, "Blessed %s with deflect for %0.1f seconds.\n", V_GetMonsterName(target->mtype), duration);
		}
...
{  
	...
	{   
		...
		T_Damage(target, ent, ent, vec3_origin, vec3_origin, vec3_origin, total, 0, DAMAGE_ENERGY, MOD_MINDABSORB);
	}  
	
	//Cap cube count to max cubes  
	if (ent->client->pers.inventory[power_cube_index] > MAX_POWERCUBES(ent) + 200)   
		ent->client->pers.inventory[power_cube_index] = MAX_POWERCUBES(ent) + 200;  
}
