	...
	for (i=0; i < MAX_CLIENTS; i++) 
	{
		...
		if (self->monsterinfo.dmglist[i].player)
		{
			...
			if (self->client)
				...
				gi.dprintf("(%s) slot %d: %s, %.0f damage (%.1f%c)\n", 
					V_GetMonsterName(self->mtype), i, slot->player->client->pers.netname, slot->damage, percent, '%');
			else
				gi.dprintf("(%s) slot %d: %s, %.0f damage (%.1f%c)\n", 
...
void boss_regenerate (edict_t *self)
{
	...
	health = self->max_health/regen_frames/10; //decino: added the /10 for less extreme regen
