...
void p_medic_heal (edict_t *ent)
{
	...
	if (G_EntExists(tr.ent))
	{
		...
		if ((tr.ent->deadflag != DEAD_DEAD) && (tr.ent->health > 0))
		{
			...
			// heal them
			M_Regenerate(tr.ent, frames, 0, true, true, false, &tr.ent->monsterinfo.regen_delay2);

			// remove all curses
			CurseRemove(tr.ent, 0);

			if (tr.ent->client)
			tr.ent->client->is_poisoned = false;
...
void p_medic_regen (edict_t *ent)
{
	...
	M_Regenerate(ent, health_r_frames, MEDIC_REGEN_DELAY, true, false, false, &ent->monsterinfo.regen_delay1);
}
