...
//			gi.dprintf("You are a spectator, no exp for you.\n");
		    return;}
...
void info_player_invasion_death (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	//decino: added a message for a destroyed spawn
	G_PrintGreenText(va("A human spawn was destroyed by a %s!", V_GetMonsterName(attacker->mtype)));
...
void info_player_invasion_think (edict_t *self)
{
	if (level.time > self->lasthurt + 1.0)
		M_Regenerate(self, PLAYERSPAWN_REGEN_FRAMES, PLAYERSPAWN_REGEN_DELAY, true, false, false, &self->monsterinfo.regen_delay1);
