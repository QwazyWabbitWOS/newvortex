...
int p_berserk_melee (edict_t *self, vec3_t forward, vec3_t dir, int damage, int knockback, int range, int mod)
{
	...
	//Talent: Melee Increase for polts
	talentLevel = getTalentLevel(self, TALENT_SECOND_CHANCE);
	range = 64 + 6.4*talentLevel;
...
#define BERSERK_SLASH_INITIAL_DAMAGE	50
#define BERSERK_SLASH_ADDON_DAMAGE		15
...
#define BERSERK_CRUSH_INITIAL_DAMAGE	50
#define BERSERK_CRUSH_ADDON_DAMAGE		15
...
void RunBerserkFrames (edict_t *ent, usercmd_t *ucmd)
{
	...
	if (level.framenum >= ent->count)
	{
		...
		M_Regenerate(ent, regen_frames, BERSERK_REGEN_DELAY, true, false, false, &ent->monsterinfo.regen_delay1);
...
void Cmd_PlayerToBerserk_f (edict_t *ent)
{
	...
	if(getTalentSlot(ent, TALENT_MORPHING) != -1)
		cost *= 1.0 - 0.32 * getTalentLevel(ent, TALENT_MORPHING);
