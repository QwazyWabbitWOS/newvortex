...
//			return save;
		...
		if (cl_ent && (cl_ent->myskills.abilities[POWER_SHIELD].current_level > 0))
			damagePerCell = 0.1 + 0.15*cl_ent->myskills.abilities[POWER_SHIELD].current_level; //changed 0.19 to 0.15
		else	
			damagePerCell = 1.0;
...
//decino: we stole shit, show it!
void ammosteal_sparks (edict_t *self, int num, int radius)
{
	int		i;
	vec3_t	start;
	...
	for (i=0; i<num; i++)
	{
		...
		gi.WriteByte(GetRandom(73, 90)); // color
...
//		if(totem && x < totem->monsterinfo.level * EARTHTOTEM_DAMAGE_MULT)
		...
		if (totem)
			damage *= (1.0 + EARTHTOTEM_DAMAGE_MULT * totem->monsterinfo.level);
	}

	// try to deflect an attack first before all other resistances are checked
	// extra code line (hopefully this line will fix the svn error?)
	DeflectHitscan(targ, inflictor, attacker, dir, point, normal, &damage, knockback, dflags, mod);

	//decino: this is for checking resistances
	...
	if(target_has_pilot || targ->client)
	{
		...
		{
			int resistLevel = getTalentLevel(totem->activator, TALENT_WIND);
			if(x < resistLevel * EARTHTOTEM_RESIST_MULT)
				damage = startDamage * (1.0 - 0.083 * resistLevel);
		}
	}
	...
	//	gi.dprintf("lower resist = %.1f resist = %.1f result = %.1f startdamage = %d", lowerResistFactor, resistFactor, result, damage);	
		if (damage > 0 && mod != MOD_CRIPPLE && mod != MOD_PLAGUE)
...
//	gi.dprintf("%d damage\n", damage);
	...
	if (targ->client && (talentLevel = getTalentLevel(targ, TALENT_MAG_BOOTS)) > 0)
		knockback *= 1.0 - 0.30 * talentLevel;
...
//GHz START
	if (damage > 0)
	{
		...
		if (player)
		{
			...
			if (level.time-player->lastdmg <= 0.2)				
			...
			if (!(player->myskills.abilities[ID].disable) && (player->myskills.abilities[ID].current_level))
			{
				if (player->dmg_counter)
					...
					player->client->ps.stats[STAT_ID_DAMAGE] = damage;
			}
...
// figure momentum add
	...
	//decino: bots deal 2x more damage against non clients (unless activator is a bot)
	//(to prevent clients spreewarring to 100 because bots are dumb as hell)
	if (attacker->ai.is_bot && !targ->client)
	{
		if (targ->activator && !targ->activator->ai.is_bot)
		damage *= 2;
	}
	...
	if ((dtype & D_PHYSICAL) && HitTheWeapon(targ, attacker, point, take, dflags) && (damage > 0) && (targ->health < targ->max_health*0.5)) //apple
	{
...
// do the damage
	...
	//4.1 Darkness totem gives players a seperate vampire effect.
		...
		if(player && (attacker_has_pilot || attacker->client))
		{
			...
			int maxHP = MAX_HEALTH(player);

			if(totem != NULL)
			{
				//Talent: Shadow. Players can go beyond their "normal" max health.
				maxHP *= 1.0 + 1.67 * getTalentLevel(totem->activator, TALENT_SHADOW);

				if(player->health < maxHP)
				{
					player->health += totem->monsterinfo.level * take * DARKNESSTOTEM_VAMP_MULT;

					if (player->health += totem->monsterinfo.level * take * DARKNESSTOTEM_VAMP_MULT < 1){
					if (random() < 0.5)
						player->health += 1;}
		...
		if (CanUseVampire(targ, attacker, dflags, mod))
		{
			...
			if (attacker->mtype)
			{
				...
				// mutants can vamp more than maximum health
				if (attacker->mtype == MORPH_MUTANT)
					max_health *= 1.5;
			}
		...
		ability = &attacker->myskills.abilities[FURY];                                           //decino: victim must be alive to activate attacker's fury
		if(attacker != targ && dtype & D_PHYSICAL && !(dtype & D_MAGICAL) && ability->current_level > 0 && ability->delay < level.time && targ->health > 0)
		{
		//	int chance = ability->current_level;
			...
			attacker->checkfurydmg += damage;
			chance = 4 * ability->current_level;

			talentLevel = (getTalentLevel(attacker, TALENT_PEACE));

			if (attacker->checkfurydmg >= 800 - talentLevel*150)
			{

			//reset
			attacker->checkfurydmg = 0;

			if(r < chance)
			{
				attacker->fury_blendtime = level.time + 0.2;
...

			{
                ...
                ability->delay = level.time + 0.1;
			}
		}

	talentLevel = (getTalentLevel(attacker, TALENT_TACTICS));

	//decino: poison talent
	//target can't be poisoned twice and another person can't steal poison damage
	if (attacker != targ && dtype & D_PHYSICAL && !(dtype & D_MAGICAL) && targ->client && !targ->client->is_poisoned && talentLevel > 0 && targ->health > 0)
	{
		...
		if (attacker->client)
		{
			...
			if (r < chance)
			{
				targ->client->poisonedby = attacker;
				targ->client->is_poisoned = true;

				//gi.dprintf("You have poisoned someone!\n");

				//Spawn some initial green blood to indicate the poison
				SpawnDamage(TE_SPLASH, targ->s.origin, vec3_origin, damage);
			}
		}
	}

	talentLevel = (getTalentLevel(attacker, TALENT_OVERLOAD));

	//decino: ammo steal talent
	//check for ammo steal
	if (attacker != targ && dtype & D_PHYSICAL && !(dtype & D_MAGICAL) && targ->client && talentLevel > 0 && targ->health > 0)
	{
		//ensure that poison is enabled for clients
		if (attacker->client)
		{
			...
			//these weapons have alot of damage chance (in terms of frames/pellets)
			if  (  attacker->client->pers.weapon == FindItem("Sword")
				|| attacker->client->pers.weapon == FindItem("Shotgun")
				|| attacker->client->pers.weapon == FindItem("Super Shotgun")
				|| attacker->client->pers.weapon == FindItem("Machinegun")
				|| attacker->client->pers.weapon == FindItem("Chaingun")
				|| attacker->client->pers.weapon == FindItem("BFG10k"))
			randomness = 5;

			//these weapons can be spammed
			if  (  attacker->client->pers.weapon == FindItem("HyperBlaster")
                || attacker->client->pers.weapon == FindItem("20MM Cannon"))
			randomness = 10;

			//these weapons require a bit of aim
			if (   attacker->client->pers.weapon == FindItem("Grenade Launcher")
			   ||  attacker->client->pers.weapon == FindItem("Rocket Launcher")
			   ||  attacker->client->pers.weapon == FindItem("Blaster")
			   ||  attacker->client->pers.weapon == FindItem("Railgun"))
		    randomness = 40;

			//grenade users deserve it :>
			if (   attacker->client->pers.weapon == FindItem("Grenades"))
			randomness = 90;

			//default amount of ammo stolen
            chance = 0.0005 * damage;
			...
			//for pellet based weapons that deal shit damage
			if (chance < 0.1)
				chance = 0.1;

			//never steal too much
			if (chance > 0.5)
				chance = 0.5;

			if (r < randomness){

			//decino: taken from the ammo steal (spell) command
		    //Start stealing ammo
		    shells	+= targ->client->pers.inventory[ITEM_INDEX(FindItem("Shells"))]	* chance;
		    bullets += targ->client->pers.inventory[ITEM_INDEX(FindItem("Bullets"))]	* chance;
		    rockets += targ->client->pers.inventory[ITEM_INDEX(FindItem("Rockets"))]	* chance;
		    slugs	+= targ->client->pers.inventory[ITEM_INDEX(FindItem("Slugs"))]		* chance;
		    cells	+= targ->client->pers.inventory[ITEM_INDEX(FindItem("Cells"))]		* chance;
		
		    //This next part does the same thing as subtracting the amount stolen from the target
		    targ->client->pers.inventory[ITEM_INDEX(FindItem("Shells"))]	*= (1 - chance);
		    targ->client->pers.inventory[ITEM_INDEX(FindItem("Bullets"))]	*= (1 - chance);
		    targ->client->pers.inventory[ITEM_INDEX(FindItem("Rockets"))]	*= (1 - chance);
		    targ->client->pers.inventory[ITEM_INDEX(FindItem("Slugs"))]	*= (1 - chance);
		    targ->client->pers.inventory[ITEM_INDEX(FindItem("Cells"))]	*= (1 - chance);

			//Add what we stole from the other people. :)
	        attacker->client->pers.inventory[ITEM_INDEX(FindItem("Shells"))] += shells;
	        attacker->client->pers.inventory[ITEM_INDEX(FindItem("Bullets"))] += bullets;
	        attacker->client->pers.inventory[ITEM_INDEX(FindItem("Rockets"))] += rockets;
	        attacker->client->pers.inventory[ITEM_INDEX(FindItem("Slugs"))] += slugs;
	        attacker->client->pers.inventory[ITEM_INDEX(FindItem("Cells"))] += cells;
	        Check_full(attacker);//Make sure we are not over our limits

//			gi.cprintf(targ, PRINT_HIGH, "%s just stole some of your ammo!\n", attacker->client->pers.netname);
//			gi.cprintf(attacker, PRINT_HIGH, "You just stole %d%% of %s's ammo!\n", (chance * 100), targ->client->pers.netname);


			//spawn some sparks to indicate successfull ammo steal
	        for (i=0; i<10; i++)
	        {
		    VectorCopy(targ->s.origin, start);
		    start[0] += crandom() * GetRandom(0, 24);
		    start[1] += crandom() * GetRandom(0, 24);
		    start[2] += crandom() * GetRandom(0, 24);
		    ...
		    gi.WritePosition(start);
		    gi.WriteDir(vec3_origin);
		    gi.WriteByte(GetRandom(220, 224)); // color (yellow-ish)
		    gi.multicast(start, MULTICAST_PVS);
	        }
		}
	}
...
void CheckPoisonDamage(edict_t *ent)
{
	int dmg;
	int talentLevel;

	//decino: level 3 poison is equal to level 6 plague
	talentLevel = (getTalentLevel(ent->client->poisonedby, TALENT_TACTICS));
	dmg = (float)talentLevel/5 * ((float)ent->max_health/20);

	if (ent->client->is_poisoned)
	{
		//if the character that poisoned us is gone then remove poison
		if (ent->client->poisonedby && (!ent->client->poisonedby->inuse || !ent->client->poisonedby->client || ent->client->poisonedby->client->resp.spectator))
		{
			ent->client->poisonedby = NULL;
...
//		T_Damage(ent, ent->character->poisonedby, ent->character->poisonedby, vec3_origin, *CenterEdict(ent), NULL, 10, 0, DAMAGE_NO_RESIST | DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK , MOD_POISON);
		T_Damage(ent, ent->client->poisonedby, ent->client->poisonedby, vec3_origin, ent->s.origin, vec3_origin, dmg, 0, DAMAGE_NO_ABILITIES, MOD_POISON); // hurt 'em
	}
} 
