...
void SaveCharacter (edict_t *ent)
{
	...
	if(Q_strcasecmp(oldPassword, newPassword) != 0)
	{
		if (gds->value)		WriteToLogfile(ent, "Changed password.\n");
		else				WriteToLogfile(ent, va("Changed password from '%s' to '%s'.\n", oldPassword, newPassword));

		//decino: in case people forget their new password
		//FIXME: somehow make this work when someone uses 'reconnect'?
		//gi.cprintf(ent, PRINT_HIGH, "INFO: You've changed your password from '%s' to '%s'!\nIf you ever forget it check out your condumps folder.\n", oldPassword, newPassword);
