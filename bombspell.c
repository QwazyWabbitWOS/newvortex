...
void BombArea (edict_t *ent, float skill_mult, float cost_mult)
{
	...
	bomb->owner = ent;
	bomb->delay = level.time + BOMBAREA_DURATION;
	bomb->nextthink = level.time + 2;
	...
	gi.sound(ent, CHAN_ITEM, gi.soundindex("spells/meteorlaunch.wav"), 1, ATTN_NORM, 0);
	ent->client->pers.inventory[power_cube_index] -= cost;

	ent->client->ability_delay = level.time + BOMBAREA_DURATION*cost_mult;
}
