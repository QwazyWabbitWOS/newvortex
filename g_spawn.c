...
void G_FindTrainTeam()
{
	...
	for ( i=maxclients->value+1 ; i<globals.num_edicts ; i++, e++)
	{
		...
		{
			if(!Q_strcasecmp(e->classname,"path_corner") && e->targetname && e->target)
			{
...
//		orgtargetname = e->targetname;
				...
				while(lc < MAX_EDICTS)
				{
					...
					for ( j=maxclients->value+1 ; j<globals.num_edicts ; j++, t++)
					{
						...
						{
							if(!Q_strcasecmp(t->classname,"func_train") 
								&& !Q_strcasecmp(t->target,currtargetname)
								&& t->trainteam == NULL)
							{
								for(k = 0;k < lc; k++)
								{
					...
					{
						if(!Q_strcasecmp(targethist[k],currtargetname)) break;
					}
					if(k < loopindex)
					{
						findteam = true;
					...
					loopindex++;
					/*if(!Q_strcasecmp(currtarget,orgtargetname))
					{
						findteam = true;
						break;
					}*/
