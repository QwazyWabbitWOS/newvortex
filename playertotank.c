...
void PM_Effects (edict_t *ent)
{
	...
	// detector effects
	if (ent->flags & FL_DETECTED && ent->detected_time > level.time && ent->mtype != M_FORCEWALL)
	{
...
void p_tank_think (edict_t *self)
{
	...
	M_Regenerate(self, regenFrames, delayFrames, true, false, false, &self->monsterinfo.regen_delay1);
...
void p_tank_spawn (edict_t *ent, int cost)
{
	...
	tank->max_health = MAX_HEALTH(ent);
	tank->mtype = P_TANK;
...
*/
	...
	if(getTalentSlot(ent, TALENT_MORPHING) != -1)
		tank_cubecost *= 1.0 - 0.32 * getTalentLevel(ent, TALENT_MORPHING);
