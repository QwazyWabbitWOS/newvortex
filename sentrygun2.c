...
void sentFireRocket(edict_t *self)
{
	...
	if (que_typeexists(self->curses, AURA_HOLYFREEZE) && (!self->firewall_weak || !self->firewall_medium || !self->firewall_strong))
...
{ 
	...
	case M_SENTRY:
		...
		if( (level.time >= self->delay) && (self->style >= SENTRY_ROCKETCOST) && (self->orders >= 3))
			sentFireRocket(self);
		// are we affected by holy freeze?
		if (que_typeexists(self->curses, AURA_HOLYFREEZE)/*HasActiveCurse(self, AURA_HOLYFREEZE)*/ && !(level.framenum%2) && (!self->firewall_weak || !self->firewall_medium || !self->firewall_strong))
			break;
...
void sentrygun_think (edict_t *self)
{
	...
	if (slot && (!self->firewall_weak || !self->firewall_medium || !self->firewall_strong))
	{
	...
	if(self->chill_time > level.time)
		self->yaw_speed *= 1 / (1 + 0.1 * self->chill_level);
...
void SpawnSentry1 (edict_t *ent, int sentryType, int cost, float skill_mult, float delay_mult)
{
	...
	sentry->monsterinfo.level = ent->myskills.abilities[BUILD_SENTRY].current_level * skill_mult;
	sentry->monsterinfo.control_cost = 3;
	...
	*/
	...
	if (talentLevel == 1)
	sentry->firewall_weak = true;
	if (talentLevel == 2)
	sentry->firewall_medium = true;
	if (talentLevel == 3)
	sentry->firewall_strong = true;
