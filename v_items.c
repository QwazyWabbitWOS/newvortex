...
edict_t *V_SpawnRune (edict_t *self, edict_t *attacker, float base_drop_chance, float levelmod)
{
	int		iRandom;
	int		targ_level;
	int		greedBonus = 10*attacker->myskills.talents.GreedLevel;
	float	temp = 0;
	gitem_t *item;
	edict_t *rune;
	...
	{
		//monsters have 1/5 the chance to spawn a rune
		if (IsABoss(self) || (self->mtype == M_COMMANDER))
			temp = (float) (self->monsterinfo.level + 1) / (attacker->myskills.level + 1);
		else
			temp = (float) (self->monsterinfo.level + 1) / (attacker->myskills.level + 1) / 5.0;
	...
	if (iRandom < CHANCE_UNIQUE+greedBonus)
	{
		//spawn a unique
...
void SpawnRune (edict_t *self, edict_t *attacker, qboolean debug)
{
	...
	//TALENT: talent which increases drop rate of runes
	//0.01 + 2/3/4% more.
	if(attacker->myskills.talents.GreedLevel > 0){
	RuneSpawnRate += attacker->myskills.talents.GreedLevel * 0.00667;
	/*gi.dprintf("SPAWN RATE: %f\n", RuneSpawnRate);*/}

	if(!self->client && self->activator && self->svflags & SVF_MONSTER)
	{
		if (IsABoss(self) || (self->mtype == M_COMMANDER))
		//boss almost always drops a rune
			temp = (float) (self->monsterinfo.level + 1) / (attacker->myskills.level + 1) * 100.0;
		else
		//monsters have 1/10th the chance to spawn a rune
			temp = (float) (self->monsterinfo.level + 1) / (attacker->myskills.level + 1) / 10.0;
		
		//gi.dprintf("%.1f\n", temp*RUNE_SPAWN_BASE);

		if (RuneSpawnRate * temp < random())
			return;
...
void PurchaseRandomComboRune(edict_t *ent, int runetype)
{
	int	cost;
	edict_t *rune;
	item_t *slot;
	...
	cost *= 0.7 - 0.1 * ent->myskills.talents.BarteringLevel;
...
qboolean Pickup_Rune (edict_t *ent, edict_t *other)
{
	...
	//decino: bots can't pickup runes, they can't use them!
	if (other->ai.is_bot)
		return false;
	...
	if (ent->vrxitem.itemLevel < 1 && !ent->is_unique)
	{
		G_FreeEdict(ent);
		return false;
	}
...
void V_EquipItem(edict_t *ent, int index)
{
	...
	total_pts = ceil(0.5*wpts + 0.75*apts);//was 0.66,2.0 *** and was 1.5 in 4.4
