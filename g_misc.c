...
void ThrowClientHead (edict_t *self, int damage)
{
	...
	//decino: don't 'leave' player corpses
...
void FindMonsterSpot (edict_t *self)
{
	...
	int		total_monsters, max_monsters;
	int		mtype=0, num=0, i=0;

	total_monsters = PVM_TotalMonsters(self);

	if (pvm->value)
		max_monsters = pvm_monstermult->value*players;
	else
		max_monsters = ffa_monstermult->value*players;

	// dm_monsters cvar sets minimum number of monsters that will spawn
	if (max_monsters < dm_monsters->value)
		max_monsters = dm_monsters->value;

	// if nobody is on the server, then remove all monsters
	if (players < 1)
	{
		if (total_monsters)
		{
		...
		self->nextthink = level.time + 1; // keep looking for players
		return;
	}
	// spawn monsters until we reach the limit
	else if (total_monsters < max_monsters)
	{
		SpawnDrone(self, GetRandom(1, 7), true);
		self->nextthink = level.time + FRAMETIME;
		return;	
	}
	
	WriteServerMsg(va("World has %d/%d monsters.", total_monsters, max_monsters), "Info", true, false);
	...
	// we have enough monsters
	if (pvm->value)
		self->nextthink = level.time + pvm_respawntime->value;
	else
		self->nextthink = level.time + ffa_respawntime->value;
}
...
//	gi.dprintf("VortexRemovePlayerSummonables()\n");
	...
	self->bot_aura = false;
	self->bot_spirit = false;
	self->bot_morphed = false;
	self->bot_hellspawn = false;
	...
	for (from=g_edicts; from<&g_edicts[globals.num_edicts]; from++)
	{
		...
		if ((from->creator) && (from->creator == self) 
			&& !Q_strcasecmp(from->classname, "Sentry_Gun") && !RestorePreviousOwner(from))
		{
		...
		if ((from->activator) && (from->activator == self) 
			&& !Q_strcasecmp(from->classname, "drone") && !RestorePreviousOwner(from))
		{
		...
		// remove force wall
		if ((from->activator) && (from->activator == self) 
			&& !Q_strcasecmp(from->classname, "Forcewall"))
		{
			from->think = BecomeTE;
		...
		if ((from->owner) && (from->owner == self) 
			&& !Q_strcasecmp(from->classname, "grenade"))
		{
			self->max_pipes = 0;
			G_FreeEdict(from);
			continue;
	...
	{
		self->supplystation->think = BecomeExplosion1;
		self->supplystation->nextthink = level.time + FRAMETIME;
		self->supplystation->takedamage = DAMAGE_NO;
		self->supplystation->deadflag = DEAD_DEAD;
		self->supplystation = NULL;
	}
