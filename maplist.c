...
//gi.dprintf("arg count 2\n");
      if (Q_strcasecmp(gi.argv(MLIST_ARG_1), "HELP") == 0) 
      { 
         DisplayMaplistUsage(ent); 
         break; 
      } 
       if (Q_strcasecmp(gi.argv(MLIST_ARG_1), "START") == 0) 
      { 
      ...
      else if (Q_strcasecmp(gi.argv(MLIST_ARG_1), "NEXT") == 0) 
      { 
         if (maplist.nummaps > 0)  // does a maplist exist? 
            EndDMLevel(); 
         else 
...
//gi.dprintf("arg count 3\n");
	  ...
	  { 
		...
		if (Q_strcasecmp(gi.argv(MLIST_ARG_1), "GOTO") == 0)
		{ 
