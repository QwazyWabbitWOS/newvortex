...
void mymedic_fire_blaster (edict_t *self)
{
	...
	damage = 10 + 0.5*self->monsterinfo.level;

	MonsterAim(self, 0.9, 1500, false, MZ2_MEDIC_BLASTER_1, forward, start);
	monster_fire_blaster(self, start, forward, damage, 1500, effect, BLASTER_PROJ_BOLT, 2.0, bounce, MZ2_MEDIC_BLASTER_1);
}
...
void mymedic_cable_attack (edict_t *self)
{
	...
	if (M_NeedRegen(self->enemy))
	{
		...
		self->enemy->client->is_poisoned = false;

		//Give them a short period of curse immunity
		self->enemy->holywaterProtection = level.time + 2.0; //2 seconds immunity

		// heal them
		M_Regenerate(self->enemy, frames, 0, true, true, false, &self->enemy->monsterinfo.regen_delay2);
	...
	else if (self->enemy->health < 1 && !strcmp(self->enemy->classname, "drone"))
	{
		vec3_t bmin, bmax;

		// if this is a player-owned medic, check for max monsters
		if (self->activator->client && (self->enemy->monsterinfo.control_cost 
			+ self->activator->num_monsters > MAX_MONSTERS))
			return;

		self->enemy->monsterinfo.level = self->monsterinfo.level;
		
		// make sure the resurrected monster will fit
		M_SetBoundingBox(self->enemy->mtype, bmin, bmax);
		self->enemy->activator = self->activator; // transfer ownership!
		if (G_IsValidLocation(self->enemy, self->enemy->s.origin, bmin, bmax) && M_Initialize(self->activator, self->enemy))
		{
			self->activator->num_monsters += self->enemy->monsterinfo.control_cost;

			// restore this drone
			self->enemy->monsterinfo.slots_freed = false; // reset freed flag
			self->enemy->health = 0.2*self->enemy->max_health;
			self->enemy->monsterinfo.power_armor_power = 0.2*self->enemy->monsterinfo.max_armor;
			self->enemy->nextthink = level.time + FRAMETIME;
			self->enemy->monsterinfo.resurrected_time = level.time + 10.0; // don't award points for killing a monster so quickly after medic resurrects it
			gi.linkentity(self->enemy);
			self->enemy->monsterinfo.stand(self->enemy);
			
			if (invasion->value && !self->activator->client)
			{
				self->enemy->monsterinfo.aiflags &= ~AI_STAND_GROUND;
				self->enemy->monsterinfo.aiflags |= AI_FIND_NAVI;
			}
		}
	}
}
...
//	self->monsterinfo.walk = mymedic_walk;
