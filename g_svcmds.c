...
//GHz START
#if ALLOW_ADMIN
	else if (Q_strcasecmp (cmd, "addexp") == 0) 
        SVCmd_AddExp_f ();
	else if (Q_strcasecmp (cmd, "addtalentpoints") == 0) 
        SVCmd_AddTalentPoints_f ();
	else if (Q_strcasecmp (cmd, "addcredits") == 0) 
        SVCmd_AddCredits_f ();
	else if (Q_strcasecmp (cmd, "makeadmin") == 0) 
        SVCmd_MakeAdmin_f ();
	else if (Q_strcasecmp (cmd, "makeboss") == 0) 
        SVCmd_MakeBoss_f ();
	else if (Q_strcasecmp (cmd, "spawnboss") == 0) 
		SVCmd_SpawnBoss_f();
	else if (Q_strcasecmp (cmd, "changeclass") == 0) 
        SVCmd_ChangeClass_f();
	...
	else if (Q_strcasecmp (cmd, "teams") == 0) 
        SVCmd_Teams_f ();
	else if (Q_strcasecmp (cmd, "entities") == 0) 
        SVCmd_EntityCount_f ();
	else if (Q_strcasecmp (cmd, "eventeams") == 0)
		SVCmd_BalanceTeams_f();
	else if (Q_strcasecmp (cmd, "nodes") == 0)
		SVCmd_GenerateNodes_f();
	else if (Q_strcasecmp (cmd, "spawnflag") == 0)
		SVCmd_SpawnFlag_f();
	else if (Q_strcasecmp (cmd, "exphole") == 0)
		SVCmd_ExpHole_f();
	else if (Q_strcasecmp (cmd, "resetctf") == 0)
		CTF_Init();
	else if (Q_strcasecmp (cmd, "setteam") == 0)
		SVCmd_SetTeam_f();
		else if (Q_strcasecmp (cmd, "delchar") == 0)
		SVCmd_DeleteCharacter_f();
	else if (Q_strcasecmp (cmd, "loadmaplists") == 0)
	{
