...
void Cmd_BoostPlayer(edict_t *ent)
{
	...
	if (ent->lasthurt+DAMAGE_ESCAPE_DELAY > level.time && !ent->ai.is_bot)
		return;
	...
	switch(talentLevel)
	{
	case 1:		boost_delay -= 0.5; 	break;
	case 2:		boost_delay -= 1.0;		break;
	case 3:		boost_delay -= 1.5;		break;
	default:	//Do nothing
		break;
	}
...
void LeapAttack (edict_t *ent)
{
	...
	duration = 0.33 * getTalentLevel(ent, TALENT_LEAP_ATTACK);
	...
	while ((e = findradius (e, ent->s.origin, 128)) != NULL)
	{
		...
		T_Damage(e, e, ent, forward, point, vec3_origin, 0, 200, 0, 0);//decino: 1000 knock was too much and made this talent useless
		...
		if (e->client)
		{
			...
			e->nextthink = level.time + duration*2; //decino: 2x longer stun for non-clients
	}

	ent->client->boosted = false;
}

void ce_gib_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	if (G_ValidTarget(self->owner, other, false))
	{
		T_Damage(other, self, self->owner, self->velocity, self->s.origin, 
			plane->normal, self->dmg, self->dmg, 0, MOD_CE_GIB);
...
void fire_gib (edict_t *self, vec3_t start, vec3_t dir, int damage, float stun_length, int speed)
{
	edict_t	*bolt;
	...
	//decino: each gib does 50% damage of the explosion damage
	damage *= 0.5;
	...
	bolt->owner = self;
	bolt->touch = ce_gib_touch;
	bolt->nextthink = level.time + 3;
	bolt->think = G_FreeEdict;
	bolt->dmg = damage;
	bolt->classname = "ce_gib"; //ce = corpse explode
	gi.linkentity (bolt);
}
...
void Cmd_CorpseExplode(edict_t *ent)
{
	...
	max_dmg = 100 + 30*ent->myskills.abilities[CORPSE_EXPLODE].current_level;
	min_dmg = 0.9*max_dmg;
	dmg = GetRandom(min_dmg, max_dmg);
	radius = 100 + 15.6*ent->myskills.abilities[CORPSE_EXPLODE].current_level;
	if (radius > 512)
		radius = 512;
	...
	while ((e = findclosestradius (e, tr.endpos, 256)) != NULL)
	{
		if (!G_EntExists(e))
			continue;
		if (e->health > 0)
		...
		// cause the damage
		T_RadiusDamage (e, ent, dmg, NULL, radius, MOD_CORPSEEXPLODE);
		//Spells like corpse explode shouldn't display 10000 damage, so show the corpse damage instead
		ent->client->ps.stats[STAT_ID_DAMAGE] = dmg;
...
void Vampire_Think(edict_t *self)
{
	...
	{
		if (self->deadflag != DEAD_DEAD && self->solid != SOLID_NOT && self->has_IR) //decino: check if the player has IR toggle on
		{
...
void fire_magicbolt (edict_t *ent, int damage, int radius_damage, float damage_radius)
{
	edict_t	*bolt;
...
{
	int talentLevel = getTalentLevel(ent, TALENT_IMP_MAGICBOLT);
	...
	//decino: don't increase the cost
	cost = BOLT_COST;
	cost_mult = 1;
	...
	if(talentLevel > 0)
	{
		ent->client->ability_delay = level.time+IMP_BOLT_DELAY*cost_mult;
		radius = IMP_BOLT_RADIUS;
		radius_damage = talentLevel * 0.33 * damage;
	}
	else
		ent->client->ability_delay = level.time+BOLT_DELAY*cost_mult;

	fire_magicbolt(ent, damage, radius_damage, radius);	
...
void Cmd_Nova_f (edict_t *ent, int frostLevel, float skill_mult, float cost_mult)
{
	...
	if (!G_CanUseAbilities(ent, ent->myskills.abilities[NOVA].current_level, NOVA_COST))
		return;
	...
	if(frostLevel > 0)
	{
		...
		while ((target = findradius(target, ent->s.origin, FROSTNOVA_RADIUS)) != NULL)
		{
			...
			target->chill_level = 3.33 * frostLevel;
...
void Cmd_FrostNova_f (edict_t *ent, float skill_mult, float cost_mult)
{
	...
	if(talent->upgradeLevel > 0)
	{
		if(talent->delay < level.time)
		{
			...
			talent->delay = level.time + 2.0;	//2 second recharge.
		}
...
{
	int fx=RF_SHELL_RED, team, dist=EXPLODING_ARMOR_DETECTION;
	...
	team = self->owner->teamnum;

	// determine team colors
	// since red is the default color, we don't need to put it here
	if (team == BLUE_TEAM)
		fx = RF_SHELL_BLUE;

	// flash 10 seconds before detonation
	if (level.time > (self->delay - 10) && (level.framenum % 5) == 0)
	{
		if (self->s.renderfx & fx)
		{
	...
	else if (fx) // normal shell effects
	{
		self->s.effects |= EF_COLOR_SHELL;
		self->s.renderfx |= fx;
	}
...
void proxy_remove (edict_t *self, qboolean print)
{
	...
	if (self->owner && self->owner->inuse)
	{
		self->owner->num_proxy--;

		if (print)
			safe_cprintf(self->owner, PRINT_HIGH, "%d/%d proxy grenades remaining.\n",
				self->owner->num_proxy, PROXY_MAX_COUNT);
	}
}

void proxy_explode (edict_t *self)
{
	safe_cprintf(self->owner, PRINT_HIGH, "Proxy detonated.\n");

	T_RadiusDamage(self, self->owner, self->dmg, NULL, self->dmg_radius, MOD_PROXY);
...
{
	safe_cprintf(self->owner, PRINT_HIGH, "Proxy grenade destroyed.\n");
	proxy_remove(self, true);
}
...
void SpawnProxyGrenade (edict_t *self, int cost, float skill_mult, float delay_mult)
{
	...
	tr = gi.trace (start, bmin, bmax, end, self, MASK_SOLID); //was MASK_SOLID
	...
	talentLevel = getTalentLevel(self, TALENT_PRECISION_TUNING);
	if (talentLevel == 1)
...
void napalm_think (edict_t *self)
{
	...
	// assign team colors
	if (self->owner->teamnum == RED_TEAM)
	{
		self->s.effects |= EF_COLOR_SHELL;
		self->s.renderfx |= RF_SHELL_RED;
	}
	else if (self->owner->teamnum == BLUE_TEAM)
	{
		self->s.effects |= EF_COLOR_SHELL;
		self->s.renderfx |= RF_SHELL_BLUE;
	}
...
void Cmd_Napalm_f (edict_t *ent)
{
	...
	cost = NAPALM_COST - 1.67*getTalentLevel(ent, TALENT_BOMBARDIER);
...
#define METEOR_INITIAL_DMG		200
#define METEOR_ADDON_DMG		30
#define METEOR_INITIAL_RADIUS	200
#define METEOR_ADDON_RADIUS		0
#define METEOR_INITIAL_SPEED	650
#define METEOR_ADDON_SPEED		35
...
void ChainLightning (edict_t *ent, int damage, int attack_range, int hop_range)
{
	...
	// get ending position
	VectorMA(start, attack_range, forward, end);
	...
	while (!prev_ed[0] && hops > 0)
	{
		...
		VectorMA(forward, (-2 * DotProduct(forward, tr.plane.normal)), tr.plane.normal, forward);
		VectorMA(start, attack_range, forward, end);
...
void Cmd_ChainLightning_f (edict_t *ent, float skill_mult, float cost_mult)
{
	...
	ChainLightning(ent, damage, attack_range, hop_range);
...
#define AUTOCANNON_INITIAL_HEALTH		250
#define AUTOCANNON_ADDON_HEALTH			25
#define AUTOCANNON_INITIAL_DAMAGE		100
#define AUTOCANNON_ADDON_DAMAGE			20
...
void CreateAutoCannon (edict_t *ent, int cost, float skill_mult, float delay_mult)
{
	...
	if (talentLevel == 1)
	cannon->firewall_weak = true;
	if (talentLevel == 2)
	cannon->firewall_medium = true;
	if (talentLevel == 3)
	cannon->firewall_strong = true;
...
void autocannon_setaim (edict_t *ent, edict_t *cannon)
{
	...
	if (forward[PITCH] > 270)
	{
		...
		forward[PITCH] = 10;
	cannon->s.angles[PITCH] = forward[PITCH];
...
void Cmd_IRVision_f (edict_t *ent)
{
	if (ent->has_IR){
	safe_cprintf(ent, PRINT_HIGH, "IR Vision has been disabled.\n");
	ent->has_IR = false;}

	else{
	safe_cprintf(ent, PRINT_HIGH, "IR Vision has been enabled.\n");
	ent->has_IR = true;}
}
...
void Cmd_Buffs_f (edict_t *ent)
{
	...
	safe_cprintf(ent, PRINT_HIGH, "Buff indicator has been disabled.\n");
	...
	safe_cprintf(ent, PRINT_HIGH, "Buff indicator has been enabled.\n");
	stuffcmd(ent, "score\n");
	ent->client->showbuffs = true;}
}
...
void wormhole_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if ((other == self->creator) || IsAlly(self->creator, other))
	{
		...
		//TODO: don't make the guy invisible, how about transparant with the EF_TRACKERTRAIL effect?
...
void Cmd_SpikeGrenade_f (edict_t *ent)
{
	...
	//Talent: Bombardier - reduces grenade cost
	cost = SPIKEGRENADE_COST - 1.67*getTalentLevel(ent, TALENT_BOMBARDIER);
...
#define DETECTOR_DELAY				1.0
#define DETECTOR_MAX_COUNT			3
#define DETECTOR_INITIAL_HEALTH		50
#define DETECTOR_ADDON_HEALTH		15
#define DETECTOR_INITIAL_RANGE		64
#define DETECTOR_ADDON_RANGE		19.2
#define DETECTOR_DURATION			120.0		// how long the detector will last
#define DETECTOR_FLAG_DURATION		2.0			// how long a player has the FL_DETECTED flag (WAS 1.0)
#define DETECTOR_GLOW_TIME			2.0			// seconds detector will glow after finding a target (WAS 1.0)
...
void ProjectileLockon (edict_t *proj)
{
	...
	if (proj->update_project <= level.time)
	{
	...
	proj->update_project = level.time + 1.0; //decino: increase the curve (does this work?)
...
qboolean detector_findtarget (edict_t *self)
{
	...
	while ((target = findradius(target, self->s.origin, self->dmg_radius)) != NULL)
	{
		...
		if (target->deadflag == DEAD_DEAD || target->health < 1)
			continue;
		// don't target forcewalls
		if (target->mtype == M_FORCEWALL)
			continue;
...
void detector_think (edict_t *self)
{
	...
	if (detector_findtarget(self))
	{
		...
		if (level.time > self->sentrydelay)
		{
			...
			self->sentrydelay = level.time + 2.0; //1.0
		}
	...
	if (self->mtype == M_DETECTOR)
	{
		...
		detector_findprojectile(self, "ce_gib");
...
void BuildDetector (edict_t *self, vec3_t start, vec3_t forward, int slvl, float duration, int cost, float delay_mult)
{
	...
	talentLevel = getTalentLevel(self, TALENT_PRECISION_TUNING);
	if (talentLevel == 1)
	detector->firewall_weak = true;
	if (talentLevel == 2)
	detector->firewall_medium = true;
	if (talentLevel == 3)
	detector->firewall_strong = true;

	//Talent: Alarm (laser trap)
	if (duration)
	{
	...
	self->num_detectors++;
	self->client->ability_delay = level.time + DETECTOR_DELAY * delay_mult;
	self->client->pers.inventory[power_cube_index] -= cost;
...
#define LASERTRAP_ADDON_DAMAGE		25
#define LASERTRAP_DELAY				1.0
#define LASERTRAP_COST				50
#define LASERTRAP_RANGE				128

void lasertrap_remove (edict_t *self)
{
	// reset counter
	if (self->activator && self->activator->inuse)
		self->activator->num_detectors = 0;
...
void lasertrap_removeall (edict_t *self, edict_t *owner, qboolean effect)
{
	edict_t *e;

	// remove trap, if specified
	if (self && self->inuse)
	{
		if (effect && self->solid != SOLID_NOT)
		{
			// explosion effect
			gi.WriteByte (svc_temp_entity);
	...
	// remove all other traps
	for (e=g_edicts ; e < &g_edicts[globals.num_edicts]; e++)
	{
		// find all laser traps that we own
		if (e && e->inuse && (e->mtype == M_ALARM) && (e != self) && e->activator 
			&& e->activator->inuse && e->activator == owner)
		{
...
qboolean lasernode_checkstatus (edict_t *self)
{
	trace_t tr;
	vec3_t	start;

	// remove if trap entity becomes invalid
	if (!self->creator || !self->creator->inuse)
		return false;

	// nodes self-destruct if at least one of them touches a wall
	if (gi.pointcontents(self->s.origin) & CONTENTS_SOLID)
		return false;

	// idle check
	if (!self->creator->enemy)
	{
		VectorCopy(self->s.origin, start);
		start[2] = self->absmin[2] - 1;
		tr = gi.trace(self->s.origin, NULL, NULL, start, self, MASK_SOLID);

		// nodes self-destruct if at least one of them is not on solid ground
		if (tr.fraction == 1.0)
			return false;
	}
...
void lasernode_think (edict_t *self)
{
	trace_t tr;
	vec3_t	v;

	if (!lasernode_checkstatus(self))
	{
		lasertrap_removeall(self, self->activator, true);
		return;
	}

	// trap entity has an enemy
	if (self->creator->enemy)
	{
		...
		// trace between this node and the next node
		tr = gi.trace(self->s.origin, NULL, NULL, self->owner->s.origin, self, MASK_SHOT);

		// hurt any valid targets that cross between the nodes
		if (G_ValidTarget(self, tr.ent, false))
		{
			// calculate attack vector
			VectorSubtract(tr.ent->s.origin, self->s.origin, v);
			VectorNormalize(v);

			// hurt them
			T_Damage(tr.ent, self, self, v, tr.endpos, vec3_origin, self->dmg, 0, DAMAGE_ENERGY, MOD_LASER_DEFENSE);//FIXME: change mod
		}
	}
	else
		self->flags |= FL_NOTARGET; // become invisible to AI
...
void lasertrap_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	safe_cprintf(self->owner, PRINT_HIGH, "Your laser trap was destroyed.\n");

	lasertrap_removeall(self, self->activator, true);
}

edict_t *BuildLaserNode (edict_t *self, vec3_t start)
{
	edict_t *trap;
	...
	trap->creator = self;// link to trap entity
	trap->activator = self->activator; // copy activator from trap entity
	trap->monsterinfo.level = self->monsterinfo.level; // copy level from trap entity

	trap->dmg = LASERTRAP_INITIAL_DAMAGE + 40 * self->monsterinfo.level;

	trap->classname = "lasernode";
	trap->think = lasernode_think;
	trap->nextthink = level.time + FRAMETIME;
	trap->solid = SOLID_BBOX;
	trap->takedamage = DAMAGE_AIM;
	trap->flags |= FL_NOTARGET;
	
	trap->die = lasertrap_die;
	trap->clipmask = MASK_SHOT;
	trap->mtype = M_ALARM;
	trap->health = trap->max_health = LASERTRAP_INITIAL_HEALTH + LASERTRAP_ADDON_HEALTH * self->monsterinfo.level;
...
void lasertrap_findtarget (edict_t *self)
{
	...
	while ((e = findradius(e, self->s.origin, 80)) != NULL)
	{
		if (!G_ValidTarget(self, e, true))
			continue;
		self->enemy = e;
...
void lasertrap_attack (edict_t *self)
{
	...
	// copy enemy's vertical position
	G_EntMidPoint(self->enemy, start);
	self->s.origin[2] = start[2];
	gi.linkentity(self);

	// rotate and move position of node 1
	self->selected[0]->s.angles[YAW] += 10;
	AngleCheck(&self->selected[0]->s.angles[YAW]);
	AngleVectors(self->selected[0]->s.angles, forward, NULL, NULL);
	VectorMA(self->s.origin, LASERTRAP_RANGE, forward, start);
	VectorCopy(start, self->selected[0]->s.origin);
	gi.linkentity(self->selected[0]);

	// rotate and move position of node 2
	self->selected[1]->s.angles[YAW] += 10;
	AngleCheck(&self->selected[1]->s.angles[YAW]);
	AngleVectors(self->selected[1]->s.angles, forward, NULL, NULL);
	VectorMA(self->s.origin, LASERTRAP_RANGE, forward, start);
	VectorCopy(start, self->selected[1]->s.origin);
	gi.linkentity(self->selected[1]);

	// rotate and move position of node 3
	self->selected[2]->s.angles[YAW] += 10;
	AngleCheck(&self->selected[2]->s.angles[YAW]);
	AngleVectors(self->selected[2]->s.angles, forward, NULL, NULL);
	VectorMA(self->s.origin, LASERTRAP_RANGE, forward, start);
	VectorCopy(start, self->selected[2]->s.origin);
	gi.linkentity(self->selected[2]);
}

void lasertrap_reset (edict_t *self)
{
	// reset enemy pointer
	self->enemy = NULL;
	...
	self->selected[0]->s.origin[2] = self->s.origin[2];
	gi.linkentity(self->selected[0]);
	self->selected[1]->s.origin[2] = self->s.origin[2];
	gi.linkentity(self->selected[1]);
	self->selected[2]->s.origin[2] = self->s.origin[2];
	gi.linkentity(self->selected[2]);
}
...
void lasertrap_think (edict_t *self)
{
	...
	if (!G_EntIsAlive(self->owner))
	{
		lasertrap_removeall(self, self->owner, false);
		return;
	}

	// if enemy is valid and in-range, then attack
	if (G_ValidTarget(self, self->enemy, false) && entdist(self, self->enemy) <= LASERTRAP_RANGE)
		lasertrap_attack(self);
	// if we had an enemy, reset
	else if (self->enemy)
		lasertrap_reset(self);
	// find a new target
	else
...
void lasertrap_buildthink (edict_t *self)
{
	vec3_t start, forward;
	...
	if (level.time > self->delay)
	{
		// remove if we haven't touched down yet or we are in the water
		...
		//VectorCopy(self->activator->s.angles, self->s.angles);
		...
		VectorCopy(self->s.origin, start);
		AngleVectors(self->s.angles, forward, NULL, NULL);
		VectorMA(start, LASERTRAP_RANGE, forward, start);
		self->selected[0] = BuildLaserNode(self, start);
		VectorCopy(self->s.angles, self->selected[0]->s.angles);// copy angles
		
		// build second node
		VectorCopy(self->s.origin, start);
		self->s.angles[YAW] += 120;
		AngleCheck(&self->s.angles[YAW]);
		AngleVectors(self->s.angles, forward, NULL, NULL);
		VectorMA(start, LASERTRAP_RANGE, forward, start);
		self->selected[1] = BuildLaserNode(self, start);
		VectorCopy(self->s.angles, self->selected[1]->s.angles);// copy angles
		self->selected[1]->owner = self->selected[0];// link to previous node
		...
		AngleVectors(self->s.angles, forward, NULL, NULL);
		VectorMA(start, LASERTRAP_RANGE, forward, start);
		self->selected[2] = BuildLaserNode(self, start);
		VectorCopy(self->s.angles, self->selected[2]->s.angles);// copy angles
		self->selected[2]->owner = self->selected[1];// link to previous node

		// first node links to last node
		self->selected[0]->owner = self->selected[2];

		self->think = lasertrap_think;
		self->svflags |= SVF_NOCLIENT;
		self->takedamage = DAMAGE_NO;
		self->solid = SOLID_NOT;
		self->movetype = MOVETYPE_NONE;
		VectorCopy(self->s.origin, self->pos1);//save final position
	}
...
void ThrowLaserTrap (edict_t *self, vec3_t start, vec3_t aimdir, int skill_level)
{
	...
	trap->owner = trap->activator = self;
	trap->monsterinfo.level = skill_level;

	trap->classname = "lasertrap";
	trap->think = lasertrap_buildthink;
	trap->nextthink = level.time + FRAMETIME;
	trap->solid = SOLID_BBOX;
	trap->takedamage = DAMAGE_AIM;
	...
	trap->health = trap->max_health = LASERTRAP_INITIAL_HEALTH + 175 * skill_level;
...
void Cmd_Detector_f (edict_t *ent)
{
	...
	if (Q_strcasecmp (gi.args(), "remove") == 0)
	{
		...
		lasertrap_removeall(NULL, ent, false);//4.4 Talent: Alarm
		return;
	}
...
void Cmd_LaserTrap_f (edict_t *ent)
{
	...
	if (Q_strcasecmp (gi.args(), "remove") == 0)
	{
		...
		detector_removeall(ent);
		lasertrap_removeall(NULL, ent, false);
		return;
	}
	...
	if (print)
	{
		if (old_owner->client)
			safe_cprintf(old_owner, PRINT_HIGH, "Your %s was converted by %s (%d/%d)\n", 
				V_GetMonsterName(other->mtype), ent->client->pers.netname, current_num, max_num);

		if (ent->client)
			safe_cprintf(ent, PRINT_HIGH, "A level %d %s was converted to your side for %.0f seconds\n", other->monsterinfo.level, V_GetMonsterName(other->mtype), duration);
	}
...
void Cmd_Conversion_f (edict_t *ent)
{
	...
	//	gi.dprintf("random = %f chance = %f\n", r, chance);
		...
		if (e->firewall_weak)
        duration *= 0.75;
        if (e->firewall_medium)
        duration *= 0.50;
        if (e->firewall_strong)
        duration *= 0.25;
...
void DeflectProjectile (edict_t *self, char *className, float chance, qboolean in_front)
{
	...
	while((e = G_Find(e, FOFS(classname), className)) != NULL)
	{
		...
		{
			// Detector code shows a possible path where mtype is set to M_ALARM, so I check that as well
			if((e->owner->mtype == M_DETECTOR) || (e->owner->mtype == M_ALARM))
...
//		return;
	...
	if (MirroredEntitiesExist(ent))
	{
		...
		safe_cprintf(ent, PRINT_HIGH, "Decoys removed.\n");
		return;
	}
...
void fire_icebolt (edict_t *self, vec3_t start, vec3_t aimdir, int damage, float damage_radius, int speed, int chillLevel, int chillDuration)
{
	edict_t	*icebolt;
	vec3_t	dir;
	vec3_t	forward;
...
void Cmd_IceBolt_f (edict_t *ent, float skill_mult, float cost_mult)
{
	...
	int		damage, speed, chill_duration, cost=ICEBOLT_COST*cost_mult;
	float	radius;
	vec3_t	forward, right, start, offset;
	...
	damage = ICEBOLT_INITIAL_DAMAGE + 50 * slvl;
	radius = ICEBOLT_INITIAL_RADIUS + 25 * slvl;
	speed = ICEBOLT_INITIAL_SPEED + 100 * slvl;
	chill_duration = ICEBOLT_INITIAL_CHILL_DURATION + ICEBOLT_ADDON_CHILL_DURATION * skill_mult;
...
void fire_plasmabolt (edict_t *self, vec3_t start, vec3_t aimdir, int damage, float damage_radius, int speed, float duration)
{
	...
	bolt = G_Spawn();
	VectorCopy (start, bolt->s.origin);
	VectorSet(bolt->velocity,0,0,-300);
    VectorSet(bolt->avelocity,900+crandom()*10.0,900+crandom()*10.0,900+crandom()*10.0);
	bolt->movetype = MOVETYPE_WALLBOUNCE;
	bolt->clipmask = MASK_SHOT;
	bolt->solid = SOLID_BBOX;
	bolt->s.effects |= EF_BLASTER | EF_TRACKER | EF_COLOR_SHELL;
	bolt->s.renderfx |= RF_SHELL_GREEN | RF_TRANSLUCENT;
...
{
	int damage, talentLevel = getTalentLevel(self->activator, TALENT_EXPLODING_BODIES);

	if (talentLevel < 1 || self->style)
		return false;

	// cause the damage
	damage = 166.67 * talentLevel;
	T_RadiusDamage (self, self, damage, self, 150.0, MOD_CORPSEEXPLODE);
...
//	gi.sound (self, CHAN_VOICE, gi.soundindex (va("spells/corpse explode %d.wav", GetRandom(1, 6))), 1, ATTN_NORM, 0);
	return true;
}
...
void healer_heal (edict_t *self, edict_t *other)
{
	...
	if (G_EntIsAlive(other) && G_EntIsAlive(self) && OnSameTeam(self, other) && other != self)
	{
		...
		// regenerate health
		//if (M_Regenerate(other, frames, 1, true, false, false, &other->monsterinfo.regen_delay2))
		//	regenerate = true;

		//health = floattoint( (float)other->max_health*1.5 / ((float)frames / 1) );
		health = self->monsterinfo.level/5;
		...
		if (talentLevel > 0)
		{
			frames *= (4.5 - (0.83 * talentLevel));
		//	if (M_Regenerate(other, frames, 10, false, true, false, &other->monsterinfo.regen_delay3))
		//		regenerate = true;

		if (other->client)
		{
			if (other->client->pers.inventory[body_armor_index] < MAX_ARMOR(self)*1.5)
			{
				armor = self->monsterinfo.level/3;
				...
				if (other->client->pers.inventory[body_armor_index] > MAX_ARMOR(self)*1.5)
					other->client->pers.inventory[body_armor_index] = MAX_ARMOR(self)*1.5;
...
#define OBSTACLE_ADDON_HEALTH			150
...
edict_t *CreateObstacle (edict_t *ent, int skill_level)
{
	...
	e->monsterinfo.nextattack = 100 - 15 * getTalentLevel(ent, TALENT_PHANTOM_OBSTACLE);
	e->monsterinfo.level = skill_level;
	e->gib_health = -250;
	e->die = obstacle_die;
	e->touch = organ_touch;
...
void gasser_attack (edict_t *self)
{
	...
	self->s.frame = GASSER_FRAMES_ATTACK_START;
...
#define COCOON_INITIAL_DURATION		50 //300		// number of frames enemy will remain in cocoon
#define COCOON_ADDON_DURATION		0//-25
#define COCOON_MINIMUM_DURATION		50
#define COCOON_INITIAL_FACTOR		1.0
#define COCOON_ADDON_FACTOR			0.05
#define COCOON_INITIAL_TIME			10
#define COCOON_ADDON_TIME			5.0//3.0
...
edict_t *CreateCocoon (edict_t *ent, int skill_level)
{
	...
	if (talentLevel > 0)
		e->monsterinfo.jumpdn = 60 - 16.67 * talentLevel;
	else
...
{
//	int talentLevel = getTalentLevel(self, TALENT_MEDITATION);
...
//	self->nextthink = level.time + FRAMETIME;
	...
	self->s.effects &= ~EF_PLASMA;
	self->client->pers.inventory[power_cube_index] += 50;
}
...
//	power_cube->nextthink = level.time + 5;
//	power_cube->count = 20 * talentLevel; 

	ent->client->pers.inventory[power_cube_index] += 50;

	if (ent->client->pers.inventory[power_cube_index] > ent->client->pers.max_powercubes*4)
		ent->client->pers.inventory[power_cube_index] = ent->client->pers.max_powercubes*4;

	if (ent->client->pers.inventory[power_cube_index] < 0)
		ent->client->pers.inventory[power_cube_index] = 0;

	ent->client->bonus_alpha = 0.2;
	gi.sound(ent, CHAN_ITEM, gi.soundindex("misc/spawn1.wav"), 1, ATTN_NORM, 0);

	ent->holdtime = level.time + 5 - talentLevel;
	ent->client->ability_delay = level.time + 5 - talentLevel;
}

#define LASERPLATFORM_INITIAL_HEALTH		0
#define LASERPLATFORM_ADDON_HEALTH			1000
#define LASERPLATFORM_INITIAL_DURATION		0
#define LASERPLATFORM_ADDON_DURATION		120.0
#define LASERPLATFORM_MAX_COUNT				3
...
//		gi.cprintf(ent, PRINT_HIGH, "You can't use abilities during pre-game.\n");
	...
	if (ent->num_laserplatforms + 1 > talentLevel)
...
void holyground_attack (edict_t *self, float radius)
{
	...
	while ((target = findradius(target, self->s.origin, radius)) != NULL)
	{
		...
		else if (self->style == HOLYGROUND_UNHOLY)
		{
			...
			{
				// deal damage
				T_Damage(target, self, self, vec3_origin, target->s.origin, vec3_origin, 50, 0, 0, MOD_CORPSEEXPLODE);
...
void CreateHolyGround (edict_t *ent, int type, int skill_level)
{
	...
	e->svflags |= SVF_NOCLIENT;
	e->monsterinfo.level = skill_level;
	e->delay = level.time + HOLYGROUND_INITIAL_DURATION + 20.0 * skill_level;
	e->style = type;
...
//		gi.cprintf(ent, PRINT_HIGH, "You can't use abilities during pre-game.\n");
	...
	{
		ent->holyground->think = BecomeTE;
		ent->holyground->nextthink = level.time + FRAMETIME;
		return;
	}
	if (ent->client->ability_delay > level.time)
	{
...
//		gi.cprintf(ent, PRINT_HIGH, "You can't use abilities during pre-game.\n");
	...
	ent->client->invincible_framenum = level.framenum + 6.67*talentLevel; //up to 2 seconds at level 5

	//Give them a short period of curse immunity
	ent->holywaterProtection = level.time + 1.67*talentLevel; //up to 5 seconds at level 5
	ent->purge_time = ent->holywaterProtection;
