...
qboolean ToggleSecondary (edict_t *ent, gitem_t *item, qboolean printmsg)
{
	...
	else
	{
		...
		if (strcmp(item->pickup_name, "Sword") == 0) {
			if (printmsg)
				safe_cprintf(ent, PRINT_HIGH, "Magic lance\n");
			return true;
		}
		if (strcmp(item->pickup_name, "Railgun") == 0) {
			if (printmsg)
...
void Weapon_Generic (edict_t *ent, int FRAME_ACTIVATE_LAST, int FRAME_FIRE_LAST, int FRAME_IDLE_LAST, int FRAME_DEACTIVATE_LAST, int *pause_frames, int *fire_frames, void (*fire)(edict_t *ent))
{
	//old hfa code
	/*
	if (que_typeexists(ent->curses, AURA_HOLYFREEZE) && !ent->FrameShot)
	{
		ent->FrameShot = 1;
		return;
	}
	*/
	...
	{
		int Continue;
		int FreezeLevel = curse->ent->owner->myskills.abilities[HOLY_FREEZE].current_level;
		//Don't allow negative hfa to screw anything up
...
void weapon_grenade_fire (edict_t *ent, qboolean held)
{
	...
	//decino: bots are dumb and don't hold grenades
	if (ent->ai.is_bot)
		speed = max_speed;
...
void Weapon_Blaster_Fire (edict_t *ent)
{
	...
	//decino: bots are dumb and don't check for blaster ammo to regenerate (todo?)
	if (!ent->ai.is_bot)
...
void weapon_20mm_fire (edict_t *ent)
{
	...
	int damage, min, max;//25 + (int) floor(2.5 * ent->myskills.weapons[WEAPON_20MM].mods[0].current_level);
	...
	if (!ent->groundentity && !ent->waterlevel){
		ent->client->ps.gunframe = 5;
		if (ent->client && !(ent->svflags & SVF_MONSTER) && !(ent->ai.is_bot))
			safe_cprintf(ent, PRINT_HIGH, "You must be on the ground to fire the 20mm cannon.\n");
		return;
	}
