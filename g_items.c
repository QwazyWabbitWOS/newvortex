...
//GHz START
	...
	if (other->client)
	other->client->is_poisoned = false;
...
void Use_PowerArmor (edict_t *ent, gitem_t *item)
{
	...
	if (ent->myskills.abilities[POWER_SHIELD].current_level < 1)
	{
...
void Tball_Aura(edict_t *owner, vec3_t origin)
{
	...
	for (i = 1; i <= game.maxclients; i++)
	{
		...
		if (other == owner)
		{
			...
			owner->svflags &= ~SVF_NOCLIENT;//decino: hopefully this can prevent the cocoon bug
...
void fire_tball (edict_t *self, vec3_t start, vec3_t aimdir, int speed, float timer)
{
	...
	//decino: throwing tballs is a stupid thing, can be abused in invasion and no one uses this in pvp
	return;
...
void Use_Tball(edict_t *ent, gitem_t *item)
{
	...
	//decino: throwing tballs is dumb
	return;
...
void SpawnWorldAmmoType (char *pickupName, int count)
{
	...
	for (i=0; i<count; i++)
	{
		...
		if (!FindValidSpawnPoint(e, false))
			G_FreeEdict(e);
		e->s.renderfx = (RF_GLOW|RF_IR_VISIBLE);//decino: world spawned ammo were not IR visible
