...
void RunCacodemonFrames (edict_t *ent, usercmd_t *ucmd)
{
	...
	//	ent->client->ability_delay = level.time + CACODEMON_DELAY; // can't use abilities
		...
		{
			frame = ent->s.frame;
//decino: don't blink, this is a pain animation anyways and looks stupid
...
void Cmd_PlayerToCacodemon_f (edict_t *ent)
{
	...
	if (ent->mtype || PM_PlayerHasMonster(ent))
	{
		...
		//decino: can't unmorph if we're a boss
		if (ent->owner->mtype == BOSS_TANK)
			return;
	...
	if(getTalentSlot(ent, TALENT_MORPHING) != -1)
		caco_cubecost *= 1.0 - 0.32 * getTalentLevel(ent, TALENT_MORPHING);
...
void EatCorpses (edict_t *ent)
{
	...
	if (G_EntExists(tr.ent) && (level.time > ent->corpseeater_time))
	{
		...
		if(talentLevel > 0)		maxhp += 0.167 * talentLevel;	//+0.1x per upgrade
		...
		else if (!OnSameTeam(ent, tr.ent))
		{
			...
			chance = 0.167 * talentLevel;
			if (talentLevel > 0 && random() <= chance)
			{
				curse_add(tr.ent, ent, BLEEDING, 30, 3.0);
				if (tr.ent->client)
...
void PlagueCloudSpawn (edict_t *ent)
{
	...
	//decino: somehow V_CanUseAbilities keeps spamming the console
...
void plague_think (edict_t *self)
{
	...
	if (!G_EntIsAlive(self->owner) || !G_EntIsAlive(self->enemy)    // someone dies
		...
		|| (self->owner->flags & FL_TELEPORTED))                                        // owner teleported
	{
		// Remove flag that indicates user recently teleported
		if (self->owner->flags & FL_TELEPORTED)
			self->owner->flags &= ~FL_TELEPORTED;
	...
	if (level.time > self->wait)
	{
        ...
        if (self->enemy->firewall_weak)
			dmg *= 0.75;
		if (self->enemy->firewall_medium)
			dmg *= 0.50;
		if (self->enemy->firewall_strong)
			dmg *= 0.25;
