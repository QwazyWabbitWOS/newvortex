...
char s1[100];
char s2[100];
#define	CURRENT_DATE				mystrdate(s1)
#define CURRENT_TIME				mystrtime(s2)
//K03 Begin
...
//apple
#define FL_TELEPORTED			0x00080000	// player teleported
//apple
...
#define MOD_SPORE			97
#define MOD_ACID			98
#define MOD_ICEBOLT			99
#define MOD_POISON          100
#define MOD_CE_GIB          101
#define MOD_AFTERMATH       102
#define MOD_BOSS_PUNCH      103
//K03 End
...
// g_weapon.c
//
void fire_yang_guided (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius, int radius_damage);
...
//ponko
	...
	qboolean    has_IR;//^sucks :)
	qboolean    passive_darkness;//decino: darkness
	qboolean    passive_nature;//decino: nature

	qboolean    bot_morphed; //decino: bot has morphed
	qboolean    bot_aura; //decino: bot using an aura
	qboolean    bot_spirit; //decino: bot using a spirit
	qboolean    bot_hellspawn; //decino: bot has spawned a hellspawn
	qboolean    bot_noplayers; //decino: there are no players in-game (observers don't count)
	qboolean    bot_checkedlevel; //decino: checked our random level when there are no players
	qboolean    bot_saidgg; //decino: we said gg
	qboolean    removed_by_world;

	float       bot_nextskill; //decino: time before bot uses a new skill
	int         bot_anger; //decino: bot is angry (dies alot), try to chat/cuss

	qboolean    fail_name;//decino: our name is "Player"
	qboolean    no_password;//decino: no password set
	qboolean    enteringPW;//decino: password prompt
	qboolean    enteringNAME;//decino: username prompt
	qboolean    ghetto_pc;//decino: software mode for black screen
	float       next_fail_msg;//decino: next message
	...
	float       totem_effect_earth;//decino: screen blend
	float       totem_effect_air;//decino: screen blend
	...
	int shots;
	float lastkill;
	int nfer; //decino: used for 2fers/3fer/4fers/5fers/etc
...
//GHz START
	...
	float		fury_time;
	float       heal_time;              //decino: used for buff indicator
	float       deflect_time;           //decino: used for buff indicator
	float       bless_time;             //decino: used for buff indicator
	float       decoy_next;             //decino: time before decoy shoots again
	float       fury_blendtime;
	float       morph_blendtime;
	...
	qboolean    firewall_weak;//lv 1 talent
	qboolean    firewall_medium;//lv 2 talent
	qboolean    firewall_strong;//lv 3 talent
...
joined_t	playerz[MAX_CLIENTS];//changed players to playerz
...
// For replacing _strdate and _strtime
