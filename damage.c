...
{
	//Talent: Pack Animal
	int		talentLevel=5;//getTalentLevel(ent, TALENT_PACK_ANIMAL);
	edict_t *e = NULL;
	...
	while ((e = findradius (e, ent->s.origin, 1024)) != NULL)
	{
		...
		if (!IsMorphedPlayer(e))
			continue;
		return 1.0 + 0.167 * talentLevel; // +10% per level
	}
				   ...
				   vec3_t point, float damage, int dflags, int mod)
{
	...
	if (dtype & D_PHYSICAL)
	{
		...
		if (!attacker->client && attacker->mtype && PM_MonsterHasPilot(attacker))
		{
			...
			int levels = attacker->owner->myskills.level/2;
			...
			if (talentLevel > 0)
			{
				temp = attacker->health / attacker->max_health;
				if (temp > 1)
					temp = 1;
				damage *= 1.0 + ((0.33*talentLevel)*(1.0 - temp));
			}

			// Talent: Superiority
			// increases damage/resistance of players against monsters
			//talentLevel = getTalentLevel(attacker->owner, TALENT_SUPERIORITY);
			  talentLevel = attacker->owner->myskills.talents.SuperLevel;
			 if (talentLevel > 0 && targ->activator && targ->svflags & SVF_MONSTER)
				damage *= 1.0 + 0.33*talentLevel;
			...
			if (talentLevel > 0)
		        damage *= 1 + 0.015*talentLevel*levels;
		...
		//decino: haste reduces damage
		if (attacker->myskills.abilities[HASTE].current_level > 0)
		{
			haste_dmg = 1.0-(attacker->myskills.abilities[HASTE].current_level)*0.02;
	...
	if (attacker->client && (attacker != targ))
	{
		...
		if (dtype & D_PHYSICAL)
		{
			...
			if (attacker->client->pers.inventory[strength_index])
			{
				...
				if (talentLevel > 0)
				{
					temp = (float)attacker->health / (float)attacker->max_health;
					if (temp > 1)
						temp = 1;
					damage *= 1.0 + ((0.33*talentLevel)*(1.0 - temp));
				}
			...
			{
				// morphed players' attacks deal more damage after level 10
				int levels = attacker->myskills.level/2;

				// Talent: Superiority
				// increases damage/resistance of morphed players against monsters
				//talentLevel = getTalentLevel(attacker, TALENT_SUPERIORITY);
				talentLevel = attacker->myskills.talents.SuperLevel;
				if (talentLevel > 0 && targ->activator && targ->svflags & SVF_MONSTER)
					damage *= 1.0 + 0.333*talentLevel;

				// TALENT_SUPERIORITY is now a new talent that increases damage/level for polts
				talentLevel = getTalentLevel(attacker, TALENT_SUPERIORITY);
				if (talentLevel > 0)
				    damage *= 1 + 0.015*talentLevel*levels;
		...
		if (physicalDamage)
		{
			...
			if (getTalentSlot(attacker, TALENT_COMBAT_EXP) != -1)
				damage *= 1.0 + 0.083 * getTalentLevel(attacker, TALENT_COMBAT_EXP);	//+5% per upgrade

			// blood of ares effect (new)
			if (getTalentSlot(attacker, TALENT_BLOOD_OF_ARES) != -1)
			{
				int level = getTalentLevel(attacker, TALENT_BLOOD_OF_ARES)*1.67;
				float temp;
				   ...
				   vec3_t point, float damage, int dflags, int mod)
{
		...
		&& (mod == MOD_EMP || mod == MOD_MIRV || mod == MOD_NAPALM || mod == MOD_GRENADE || mod == MOD_HG_SPLASH || mod == MOD_HANDGRENADE))
	{
		...
		if (mod == MOD_EMP)
			damage *= 1.0 - 0.267 * talentLevel;
		else
			damage *= 1.0 - 0.2 * talentLevel; // damage reduced to 20% at talent level 5 (explosive damage already reduced 50% in t_radiusdamage)
	}
	...
	talentLevel = targ->myskills.talents.SuperLevel;
	if (attacker->activator && (attacker->svflags & SVF_MONSTER) && talentLevel > 0)
		temp *= 1.0 + 0.333*talentLevel;

	// cocoon bonus
	if (targ->cocoon_time > level.time)
		damage /= targ->cocoon_factor;

	// player tank is super resistant
	if (targ->mtype == P_TANK)
	{
		temp = 2.0 + 0.1 * targ->monsterinfo.level;
	...
	// morphed players gain natural resistance
	if (targ->client && targ->mtype)
	{
		float	cap=0.5;//33% resistance cap

		temp = 0;

		if (targ->mtype == MORPH_MUTANT)
			temp = 0.05*targ->myskills.abilities[MUTANT].current_level;
		else if (targ->mtype == M_MYPARASITE)
			temp = 0.05*targ->myskills.abilities[BLOOD_SUCKER].current_level;
		else if (targ->mtype == MORPH_CACODEMON)
			temp = 0.05*targ->myskills.abilities[CACODEMON].current_level;
		else if (targ->mtype == MORPH_BERSERK)
		{
			temp = 0.1*targ->myskills.abilities[BERSERK].current_level;
			cap = 1.0; // 50% maximum resistance at level 10
		}
		else if (targ->mtype == MORPH_MEDIC)
			temp = 0.05*targ->myskills.abilities[MEDIC].current_level;
	...
	if(targ->client || (targ->mtype == P_TANK))
	{
		...
		if(totem != NULL)
		{
			...
			if(GetRandom(0, 99) < 10 * getTalentLevel(totem->activator, TALENT_WIND))
				return 0;
	...
	if (targ->client)
	{
		...
		if(talentLevel > 0)
			damage *= 1.0 + 0.083 * talentLevel;	//-5% per upgrade
		...
		{    
			int damage_absorbed = 0.5 * damage;    
			float absorb_mult = 3.5 - 1.1 * getTalentLevel(targ, TALENT_MANASHIELD);    
