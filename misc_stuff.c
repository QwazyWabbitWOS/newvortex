...
//	if (ent->myskills.level != 0 && ent->myskills.respawns < 1 && ent->myskills.class_num != CLASS_KNIGHT)
//		return;
...
//	if ((ent->myskills.level != 0) && (ent->myskills.class_num != CLASS_KNIGHT) 
//		&& (ent->myskills.respawns > 0))
//		ent->myskills.respawns--;
...
void Pick_respawnweapon(edict_t *ent)
{
	...
	if (ent->myskills.level > 1000 && ent->myskills.respawns < 1)
	{
		if (ent->myskills.respawn_weapon == 1)	//if already spawning with sword
			item = FindItem("Sword");			
		else item = Fdi_BLASTER;
	}
	else
	{
	...
	for (i=0 ; i<game.maxclients ; i++)
	 {
		  ...
		  if (G_IsSpectator(cl_ent))
			  continue;
		  //decino: bots aren't real players, fucks up votes and shit
		  if (cl_ent->ai.is_bot)
			  continue;
...
switch (ent->myskills.class_num)
	...
	if (ent->myskills.talents.VitLevel > 0)		
	value += ent->myskills.level * ent->myskills.talents.VitLevel;	//+1 armor per upgrade
...
//	if(talentlevel > 0)		value += ent->myskills.level * talentlevel;	//+1 health per upgrade
	...
	if (ent->myskills.talents.VitLevel > 0)
	value += (ent->myskills.level/2) * ent->myskills.talents.VitLevel;	//+0.5 health per upgrade
...
void modify_max(edict_t *ent)
{
	...
	ent->client->pers.max_powercubes = 200 + MAX_POWERCUBES(ent);
...
//		return; // don't regenerate ammo while attacking
	...
	{
		V_GiveAmmoClip(ent, ent->myskills.abilities[AMMO_REGEN].current_level * 0.2 * ammocap, AMMO_SHELLS);		//20% of a pack per point
		V_GiveAmmoClip(ent, ent->myskills.abilities[AMMO_REGEN].current_level * 0.1 * ammocap, AMMO_BULLETS);		//10% of a pack per point
		V_GiveAmmoClip(ent, ent->myskills.abilities[AMMO_REGEN].current_level * 0.1 * ammocap, AMMO_CELLS);		//10% of a pack per point
		V_GiveAmmoClip(ent, ent->myskills.abilities[AMMO_REGEN].current_level * 0.2 * ammocap, AMMO_GRENADES);	//20% of a pack per point
		V_GiveAmmoClip(ent, ent->myskills.abilities[AMMO_REGEN].current_level * 0.2 * ammocap, AMMO_ROCKETS);		//20% of a pack per point
		V_GiveAmmoClip(ent, ent->myskills.abilities[AMMO_REGEN].current_level * 0.2 * ammocap, AMMO_SLUGS);		//20% of a pack per point
