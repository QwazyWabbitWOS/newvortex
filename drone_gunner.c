...
void mygunner_refire_chain(edict_t *self)
{
	// keep firing if the enemy is valid, airborn, or outside melee range
	if (G_ValidTarget(self, self->enemy, true) && (random() <= 0.6)
		&& (!self->enemy->groundentity || (entdist(self, self->enemy) > 128)))
	{
		self->monsterinfo.currentmove = &mygunner_move_fire_chain;
		return;
	}
...
void mygunner_attack (edict_t *self)
{
	...
	if (!self->enemy->groundentity || (dist > 384))
	{
	...
	{
		if ((random() <= 0.8) && !(self->monsterinfo.aiflags & AI_STAND_GROUND))
		{
			self->monsterinfo.currentmove = &mygunner_move_runandshoot;
