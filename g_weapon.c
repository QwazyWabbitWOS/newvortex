...
return ((gi.inPVS(self->s.origin, other->s.origin)) 
	&& (gi.trace(start, vec3_origin, vec3_origin, end, self, MASK_SOLID).fraction == 1.0));
}
...
void Grenade_Explode (edict_t *ent)
{
	...
	//K03 Begin
	if (Q_strcasecmp(ent->classname, "grenade") == 0)
	{
...
void rocket_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	//K03 Begin
	int mod=MOD_ROCKET;
	if (Q_strcasecmp(ent->owner->classname, "SentryGun") == 0)
		mod = MOD_SENTRY_ROCKET;
	if (Q_strcasecmp(ent->owner->classname, "msentrygun") == 0)
		mod = MOD_SENTRY_ROCKET;
	if (Q_strcasecmp(ent->owner->classname, "Sentry_Gun") == 0)
		mod = MOD_SENTRY_ROCKET;
...
//GHz END
	...
	if(Q_strcasecmp (ent->classname, "lockon rocket") == 0)
...
void fire_20mm (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick)
{
	...
	int range = 500 + (50 * self->myskills.weapons[WEAPON_20MM].mods[1].current_level);
...
//	int         pull;
	...
	{
	//decino: 200 damage 200 radius at level 10
	T_RadiusDamage(self, self->owner, 100+15*self->myskills.weapons[WEAPON_BFG10K].mods[0].current_level, target, 200+15*self->myskills.weapons[WEAPON_BFG10K].mods[0].current_level, MOD_BFG_BLAST);
...
void bfg_idle_think (edict_t *self)
{
	...
	if (!G_EntIsAlive(self->owner) || (self->delay < level.time)) 
	{   
//		gi.dprintf("Removed myself at bfg_idle_think");
		G_FreeEdict(self);
		return;
	}
...
qboolean IsAmmoBox (edict_t *ent)
{
	return (ent && ent->inuse && (!strcmp(ent->classname, "ammo_rockets") || !strcmp(ent->classname, "ammo_grenades") 
		...
		|| !strcmp(ent->classname, "ammo_bullets") || !strcmp(ent->classname, "ammo_shells") ||!strcmp(ent->classname, "item_pack")));
}
...
void EMP_Explode (edict_t *self)
{
	...
	while ((e = findradius(e, self->s.origin, self->dmg_radius)) != NULL)
	{
		...
		else if (!strcmp(e->classname, "drone"))
		{
			...
			if (e->monsterinfo.control_cost >= 4)
				time *= 0.25;

			//decino: don't stun invalid types
			if (!strcmp(e->classname, "hellspawn"))
				time *= 0;
...
void EMP_Touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	// FIXME: this spams the bounce sound in mid air (not hitting anything)
	if (!other->takedamage)
	{
		if (random() > 0.5)
			gi.sound (ent, CHAN_VOICE, gi.soundindex ("weapons/hgrenb1a.wav"), 1, ATTN_NORM, 0);
...
void Cmd_TossEMP (edict_t *ent)
{
	...
	cost = EMP_COST - 1.67*getTalentLevel(ent, TALENT_BOMBARDIER);
...
void Cmd_TossMirv (edict_t *ent)
{
	...
	//Talent: Bombardier - reduces grenade cost
	cost = MIRV_COST - 1.67*getTalentLevel(ent, TALENT_BOMBARDIER);
...
*/
	...
	{
		max_count *= 1.0 + 0.34 * talentLevel;
	}
...
//decino: yang searches for enemies
void yang_guided_think (edict_t *self)
{
	edict_t		*target = NULL;
	edict_t		*aquire = NULL;
	vec3_t		vec;
	...
	// aquire new target
	while (( target = findradius (target, self->s.origin, 9024)) != NULL)
	{	
...
void fire_yang_guided (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius, int radius_damage)
{
	...
	rocket->owner = self->activator;
	rocket->nextthink = level.time + 0.1;
	rocket->think = yang_guided_think;
	rocket->touch = blaster_touch;
	rocket->dmg = damage;
	rocket->radius_dmg = radius_damage;
	rocket->dmg_radius = damage_radius;
