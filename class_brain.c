...
void magmine_attack (edict_t *self)
{
	...
	// adjust starting point for retarded bboxes
	if (self->enemy->mins[2] < 0)
		start[2] -= 8;
	else
		start[2] += 8;
	*/

	//4.0B16 magmine will not pull a target that is below it
...
void magmine_spawn (edict_t *ent, int cost, float skill_mult, float delay_mult)
{
	...
	if (talentLevel == 1)
	mine->firewall_weak = true;
	if (talentLevel == 2)
	mine->firewall_medium = true;
	if (talentLevel == 3)
	mine->firewall_strong = true;
}

void Cmd_SpawnMagmine_f (edict_t *ent)
{
	int cost=MAGMINE_COST;
...
void Cmd_PlayerToBrain_f (edict_t *ent)
{
	...
	if(getTalentSlot(ent, TALENT_MORPHING) != -1)
		brain_cubecost *= 1.0 - 0.32 * getTalentLevel(ent, TALENT_MORPHING);
