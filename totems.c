...
void RemoveTotem(edict_t *self)
{
	...
	self->activator->passive_darkness = false;
	self->activator->passive_nature = false;
...
void totem_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	//if (totemType == TOTEM_DARKNESS)
	//	self->activator->passive_totem = false;
	
	//if (totemType == TOTEM_NATURE)
	//	self->activator->passive_totem2 = false;
...
edict_t *NextNearestTotem(edict_t *ent, int totemType, edict_t *lastTotem, qboolean allied)
{
	...
	for (ed = g_edicts ; ed < &g_edicts[globals.num_edicts]; ed++)
	{
		...
		if (ed->mtype == TOTEM_DARKNESS){
		closestDistance = 99999;
		closestEnt = ed;
		return closestEnt;}

		if (ed->mtype == TOTEM_NATURE){
		closestDistance = 99999;
		closestEnt = ed;
		return closestEnt;}
...
void AirTotem_think(edict_t *self, edict_t *caster)
{
    edict_t *target = NULL;
	...
	target->totem_effect_air = level.time + 0.1;
	}
}

void EarthTotem_think(edict_t *self, edict_t *caster)
{
    edict_t *target = NULL;
	...
	{
	if (G_ValidAlliedTarget(self, target, true))
	target->totem_effect_earth = level.time + 0.1;
	}
}
...
void NatureTotem_think(edict_t *self, edict_t *caster)
{
	...
	//Find players in radius and attack them.
//	while ((target = findradius(target, self->s.origin, 1)) != NULL)
	for (target = g_edicts+1; target <= g_edicts+MAX_CLIENTS; target++)
//	if (target->inuse)
	{
		if (G_ValidAlliedTarget(self, target, true))
		{
			...
			if (!target->client)
			{
				...
				M_Regenerate(target, regen_frames, 50, true, true, false, &target->monsterinfo.regen_delay2);
				continue;
			}
			...
			if(*cubes < maxcubes)
			{
				...
				*cubes += getTalentLevel(caster, TALENT_SHADOW) * 15;
				if(*cubes > maxcubes)
					*cubes = maxcubes;
				isSomeoneHealed = true;
			}
	...
	if(isSomeoneHealed)
	{
		gi.sound(caster, CHAN_ITEM, gi.soundindex("items/n_health.wav"), 0.25, ATTN_NORM, 0);

//		if (caster->client)
//		caster->client->bonus_alpha = 0.15;
	}
...
void totem_general_think(edict_t *self)
{
	...
	if(self->delay < level.time)
	{
		...
		case TOTEM_AIR:		    	AirTotem_think(self, caster);		    break;//only used for screen blend
		case TOTEM_EARTH:			EarthTotem_think(self, caster);		    break;//only used for screen blend
		default:					break;
		}
	}
	
	if (level.time > self->lasthurt + 1.0)
		M_Regenerate(self, TOTEM_REGEN_FRAMES, TOTEM_REGEN_DELAY, true, false, false, &self->monsterinfo.regen_delay1);
...
//		cost *= 2;
	...
	if (totemType == TOTEM_NATURE || totemType == TOTEM_DARKNESS)
		cost = 100;

	if (totemType == TOTEM_EARTH)
		cost = 50;
	...
	if(ent->totem1)
	{
		...
		//getTalentLevel(ent, TALENT_TOTEM) < 1
		if(ent->myskills.abilities[TOTEM_MASTERY].current_level < 1)
		{
	...
	//if(!ent->myskills.abilities[TOTEM_MASTERY].disable && ent->myskills.abilities[TOTEM_MASTERY].current_level > 0)
    talentLevel = (getTalentLevel(ent, TALENT_TOTEM));

	if (talentLevel > 0)
		 totem->health = TOTEM_HEALTH_BASE * 5 + TOTEM_HEALTH_MULT * totem->monsterinfo.level * talentLevel;
	else totem->health = TOTEM_HEALTH_BASE + TOTEM_HEALTH_MULT * totem->monsterinfo.level;

	//decino: infinite radius for these totems
	if (totemType == TOTEM_DARKNESS)
    ent->passive_darkness = true;

	if (totemType == TOTEM_NATURE)
    ent->passive_nature = true;
	
	if (totemType == TOTEM_FIRE)
	{
		// fire totem is much tougher
		totem->health *= 2;//3 is too much
		// fire totem has a longer delay
		totem->delay = level.time + 1.0; //decino: 2 is too much
	}
	...
	case TOTEM_EARTH:
		...
		break;
	case TOTEM_DARKNESS:
		totem->s.effects |= EF_TRACKER;		//strange darkness effect
