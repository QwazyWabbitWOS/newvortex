...
float monster_increaseDamageByTalent(edict_t *owner, float damage)
{
	if(owner && owner->client)
	{
		...
		if(talentLevel > 0)	damage *= 1 / (1 + 0.167 * talentLevel);
		//Talent: Life Tap
		talentLevel = getTalentLevel(owner, TALENT_LIFE_TAP);
		if (talentLevel > 0) damage *= 1 + 0.083 * talentLevel;
...
{
	ent->s.effects = 0;//&= ~(EF_COLOR_SHELL|EF_POWERSCREEN|EF_PLASMA);
	ent->s.renderfx = 0;//&= ~(RF_SHELL_RED|RF_SHELL_GREEN|RF_SHELL_BLUE|RF_SHELL_YELLOW);
	...
	// team colors
	if (ent->activator && ent->activator->teamnum)
	{
		ent->s.effects |= EF_COLOR_SHELL;
		if (ent->activator->teamnum == RED_TEAM)	
			ent->s.renderfx |= RF_SHELL_RED;
		else if (ent->activator->teamnum == BLUE_TEAM)
			ent->s.renderfx |= RF_SHELL_BLUE;
	}

	// glow yellow if we are detected
	if (ent->flags & FL_DETECTED && ent->detected_time > level.time && ent->mtype != M_FORCEWALL) //decino: don't detect forcewalls
	{
	...
	// glow white on spree war
	if (SPREE_WAR && ent->activator && (ent->activator == world) && (SPREE_DUDE == world))
	{
		ent->s.effects |= EF_COLOR_SHELL;
	...
	// glow white if partially frozen
//	if (HasActiveCurse(ent, AURA_HOLYFREEZE) || HasActiveCurse(ent, CURSE_FROZEN))
...
//	if (HasActiveAura(ent, 0))
	...
	// decoy takes on the effects of its activator
	if (ent->activator && ent->activator->client && (ent->mtype == M_DECOY))
	{
	...
	// drones glow blue in PvM so you know they are on your side!
	if (pvm->value && ent->activator && ent->activator->client)
	{
		ent->s.effects |= EF_COLOR_SHELL;
		ent->s.renderfx |= RF_SHELL_BLUE;
	}
	...
	if (move->frame[index].aifunc)
		if (!(self->monsterinfo.aiflags & AI_HOLD_FRAME))
		{
			...
			if(self->chill_time > level.time)
				self->monsterinfo.scale *= 1 / (1 + 0.1 * self->chill_level);
