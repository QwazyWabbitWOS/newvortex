...
qboolean MonsterObits (edict_t *player, edict_t *monster)
{
	...
	else
	{
		...
		gi.bprintf(PRINT_MEDIUM, "%s was killed by a level %d %s\n", player->client->pers.netname, 
			monster->monsterinfo.level, V_GetMonsterName(monster->mtype));
	}
...
qboolean Monster_Obits (edict_t *victim, edict_t *attacker)
{
    ...
    switch (attacker->mtype)
    {
		...
		case M_BALANCESPIRIT:
			...
			message2 = va("'s Yang Spirit", attacker->classname);				
	...
	if (deathmatch->value || coop->value)
	{
		...
		if (attacker == self)
		{
			...
			case MOD_BFG_BLAST:
				...
				break;//decino: a break was missing here
			case MOD_CORPSEEXPLODE:
				message = "hugs his corpse";
				break;
			case MOD_AFTERMATH:
				message = "somehow felt his own aftermath";
				break;
			case MOD_BOMBS:
...
// END 14-APR-98	
			...
			case MOD_AFTERMATH:
				...
				break;
			case MOD_BOSS_PUNCH:
				message = "felt the power of";
				message2 = "'s metal punch";
				break;
			case MOD_BOMBS:
...
void VortexPlayerDeath (edict_t *self, edict_t *attacker, edict_t *inflictor)
{
	...
	//decino: this is a better spot for 2fers
	++attacker->nfer;
	attacker->lastkill = level.time + 1;

	if (attacker->client && attacker != self && attacker->nfer > 1)
	{
		message = HiPrint(va("%s got a %ifer.", attacker->client->pers.netname, attacker->nfer));
		gi.bprintf(PRINT_HIGH, "%s\n", message);
		message = LoPrint(message);
		attacker->myskills.num_2fers += attacker->nfer;
	}

	//decino: don't give negative exp in pregame
	if (level.time > pregame_time->value)
	VortexAddExp(attacker, self); // modify experience
	VortexDeathCleanup(attacker,self);

	//decino: don't drop backpacks during pregame
	if (level.time > pregame_time->value)
	{
		if (self->myskills.abilities[AMMO_STEAL].current_level > 0)
		AftermathPack(self, attacker);
		else
...
#define BOT_ANGRYKILL_MESSAGES 10
#define BOT_CALMKILL_MESSAGES 10
...
//GHz END
		...
		if (deathmatch->value && !self->client->showbuffs && !self->ai.is_bot)
				Cmd_Help_f (self);		// show scores
		// clear inventory
		memset(self->client->pers.inventory, 0, sizeof(self->client->pers.inventory));
		self->monsterinfo.lefty = 0;

		//decino: we are killing players, calm down
		if (attacker->ai.is_bot && !level.intermissiontime)
		{

			if (attacker->bot_anger > 4)
			{
				if (random() < 0.1){
				s = HiPrint(va("%s: %s", attacker->client->pers.netname, angrykill_msg[GetRandom(0, BOT_ANGRYKILL_MESSAGES)]));
				gi.bprintf(PRINT_HIGH, "%s\n", s);
				gi.sound(world, CHAN_VOICE, gi.soundindex("misc/talk.wav"), 1, ATTN_NONE, 0);}
			}

			if (attacker->bot_anger < 5)
			{
				if (random() < 0.05){
				s = HiPrint(va("%s: %s", attacker->client->pers.netname, calmkill_msg[GetRandom(0, BOT_CALMKILL_MESSAGES)]));
				gi.bprintf(PRINT_HIGH, "%s\n", s);
				gi.sound(world, CHAN_VOICE, gi.soundindex("misc/talk.wav"), 1, ATTN_NONE, 0);}
			}

			attacker->bot_anger -= 2;

			if (attacker->bot_anger < 0)
			attacker->bot_anger = 0;
		}
	}
	...
	if (self->client)
	self->client->is_poisoned = false;
	self->checkfurydmg = 0;

	// RAFAEL
	self->client->quadfire_framenum = 0;

	//decino: don't leave corpses in pregame
	if (level.time < pregame_time->value)
	{
...
void InitClientPersistant (gclient_t *client)
{
	...
	client->pers.inventory[ITEM_INDEX(FindItem("20mm Cannon"))] = 1;
...
//   xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\nxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\nxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\n
	...
	"Want to change names? Open the Vortex   \nmenu and choose 'change character' \n",
	...
	if (deathmatch->value || coop->value)
	{
		...
		//decino: newbie tips if you're below level 2
		if (self->myskills.level < 2)
		gi.centerprintf(self, "%s\n", messages[randomMT()%NEWBIE_MAX_MESSAGES]);
...
//		MorphToBrain(ent);
	...
	if (ent->myskills.abilities[POWER_SHIELD].current_level > 0)
	ent->flags |= FL_POWER_ARMOR;
...
void ClientUserinfoChanged (edict_t *ent, char *userinfo)
{
	...
	if (strcmp(ent->client->pers.netname, "Player") == 0)
	ent->fail_name = true;
	else
	ent->fail_name = false;
...
void ClientThinkstuff(edict_t *ent)
{
	...
	if (level.time >= ent->next_poison)
	{
	ent->next_poison = level.time + 1;

	if (ent->client->is_poisoned)
	CheckPoisonDamage(ent);
	}
	...
	if (!(level.framenum%4) && !(ent->client->buttons & BUTTON_ATTACK) 
	...
	if (ent->shield && (level.time > ent->shield_activate_time))
	{
		...
		else
		{
			...
			if ((talentLevel = getTalentLevel(ent, TALENT_REPEL)) > 0)
				DeflectProjectiles(ent, ((float)0.167*talentLevel), true);
	...
	if(ent->client && getTalentSlot(ent, TALENT_BASIC_AMMO_REGEN) != -1)
	{
		...
		if(talent->upgradeLevel > 0 && (talent->delay < level.time))
		{
			...
			{
			case 24:	V_GiveAmmoClip(ent, 1.0*ammocap, AMMO_ROCKETS);		break;	//rocket_index
			case 21:	V_GiveAmmoClip(ent, 1.0*ammocap, AMMO_BULLETS);		break;	//bullet_index
			case 22:	V_GiveAmmoClip(ent, 1.0*ammocap, AMMO_CELLS);		break;	//cell_index
			case 23:	V_GiveAmmoClip(ent, 1.0*ammocap, AMMO_GRENADES);	break;	//grenade_index
			case 20:	V_GiveAmmoClip(ent, 1.0*ammocap, AMMO_SHELLS);		break;	//shell_index
			case 25:	V_GiveAmmoClip(ent, 1.0*ammocap, AMMO_SLUGS);		break;	//slug_index
			default: //do nothing
				break;
			}
			
			talent->delay = level.time + 30 - talent->upgradeLevel * 8.33;	//30 seconds - 5 seconds per upgrade
		}
	}
	...
	if (ent->myskills.abilities[MIND_ABSORB].current_level > 0)
	{
		...
		if(getTalentSlot(ent, TALENT_IMP_MINDABSORB) != -1)
			cooldown -= 6.67 * getTalentLevel(ent, TALENT_IMP_MINDABSORB);
	...
	if (ent->client->pers.inventory[regeneration_index])
	{
		...
		if (ent->myskills.level <= 5)
			regenerate = M_Regenerate(e, 100, 10, true, true, false, &e->monsterinfo.regen_delay1);
		else if (ent->myskills.level <= 10)
			regenerate = M_Regenerate(e, 200, 10, true, true, false, &e->monsterinfo.regen_delay1);
		else
			regenerate = M_Regenerate(e, 300, 10, true, true, false, &e->monsterinfo.regen_delay1);
	...
	V_HealthCache(ent, (int)(0.15*ent->max_health), 5);
	V_ArmorCache(ent, (int)(0.15*MAX_ARMOR(ent)), 5);
	...
	if (!ent->myskills.abilities[CLOAK].disable) 
	{
		...
		else if (ent->movetype != MOVETYPE_NOCLIP)// don't see player-monsters
		{
			ent->client->cloaking = false;
			ent->svflags &= ~SVF_NOCLIENT;
		}
...
//GHz END
	...
	if (ent->next_fail_msg == 100){
	gi.centerprintf(ent, "Welcome to Vortex, newbie!\nChange your name by typing it\nin the above prompt and hit enter.\n");
	ent->next_fail_msg = 0;}
	...
	if (ent->client->resp.spectator != true)
	{
		...
		{
			modifier = 1 / (1 + 0.1 * ent->chill_level);
...
void ClientBeginServerFrame (edict_t *ent)
{
	...
	if ((ent->velocity[0] != 0) || (ent->velocity[1] != 0) || (ent->velocity[2] != 0))
	{
		...
		if(ent->client->cloaking && ent->client->ps.pmove.pm_flags & PMF_DUCKED)
		{
			...
			if(talentLevel > 0)
			{
				int cloak_cubecost = 4 - talentLevel;
